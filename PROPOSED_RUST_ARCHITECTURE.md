...
| Multipart/form-data + UploadFile semantics | `crates/fastapi-core/src/multipart.rs`, `crates/fastapi-core/src/extract.rs` | Partial | Parser/extractor exists, streamed request bodies are parsed incrementally (no full-body pre-buffer in extractor path), and streamed part assembly now flushes safe prefixes incrementally instead of retaining full in-flight part bytes in parser buffer. `UploadFile` supports async `read`/`write`/`seek`/`close` with temp-file spooling for large payloads. Remaining gap: avoid final full `Part` byte materialization for very large files (end-to-end streaming file semantics). |
